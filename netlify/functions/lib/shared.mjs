...
/* ---- digital ticket / invitation email (the one a guest gets once confirmed) ---- */
export function ticketArrival(s) {
  return [s.doors_open && `Doors open ${s.doors_open}`, s.last_entry && `Last entry ${s.last_entry}`].filter(Boolean).join("  ·  ");
}
export function ticketCalendar(s) {
...
export function ticketPayload(entry, settings) {
  const g = entry.guest || {};
  const cal = ticketCalendar(settings);
  return {
...
export function ticketEmailHtml(inv, url) {
...
// Mint (if needed) + optionally email the digital ticket. force=true always re-sends.
export async function issueTicket(entry, settings, url, force) {
  if (!entry.ticket_ref) { entry.ticket_ref = await nextTicketRef(settings); entry.issued_at = new Date().toISOString(); }
  if (!entry.confirmed_at) entry.confirmed_at = new Date().toISOString();
  entry.status = "issued";
  const inv = ticketPayload(entry, settings);
  let emailed = false;
  if (process.env.NOTIFY_FROM && entry.guest && entry.guest.email && (force || !entry.invitation_sent)) {
    const r = await sendEmail({
      from: fromAddress(), to: [entry.guest.email],
      subject: `Your Invitation | ${inv.event_name} — ${inv.ticket_ref}`,
      html: ticketEmailHtml(inv, url),
    });
    emailed = r.ok;
